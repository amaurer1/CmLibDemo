...
// CTextEdit
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
class CTextEdit : public CBlockEdit
{
// ELEMENT DATA
private:
...
public:
	CTextEdit() : CBlockEdit(), changeHandlerEnabled(true) {}
...
template <typename T>
requires number<T>
const AFX_MSGMAP* TNumberEdit<T>::GetMessageMap() const
{
...
template <typename T>
requires number<T>
const AFX_MSGMAP* PASCAL TNumberEdit<T>::GetThisMessageMap()
{
...
{
	if (!isBlocked() && !(GetStyle() & ES_READONLY))
	{
...
template <class T>
requires number<T>
inline BOOL TNumberEdit<T>::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	if (!isBlocked() && !(GetStyle() & ES_READONLY))
	{
	...
	return CTextEdit::OnMouseWheel(nFlags, zDelta, pt);
}
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
template <class T>
requires number<T>
...
{
	if (!isBlocked() && !CTextEdit::OnEnChange())
	{
		numberValid = true;
		return FALSE;
	}
