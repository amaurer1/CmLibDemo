...
void CLeftDlg::refreshDiagramValueEdt()
{
	...
	switch (p_viewWnd->getDiagramIndex())
	{
		case 0:
		{
			if (p_viewWnd->getDiagramColorModel() == EColorModel::rgb)
			{
				diagramValueEdt.setLimits(0, 255);
				diagramValueEdt.setIncrements(16, 1);
				diagramValueEdt.setValue(roundTo<int>(v * 255.0));
			}
			else
			{
				diagramValueEdt.setLimits(0, 360);
				diagramValueEdt.setIncrements(15, 1);
				diagramValueEdt.setValue(roundTo<int>(v * 360.0));
			}
			break;
		}
		case 1:
		{
			...
			else
			{
				...
				diagramValueEdt.setIncrements(10, 1);
				diagramValueEdt.setValue(roundTo<int>(v * 100.0));
			}
			break;
		}
		case 2:
...
{
	const EColorModel cm = static_cast<EColorModel>(diagramColorModelLsb.GetCurSel());
...
void CLeftDlg::OnDiagramValueEdtChange()
{
	...
	p_viewWnd->setDiagramValue(diagramValueEdt.getValue());
}
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
{
	const EColorModel cm = static_cast<EColorModel>(contourColorModelLsb.GetCurSel());
...
END_MESSAGE_MAP()
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
int CLeftBar::OnCreate(LPCREATESTRUCT p_cs)
{
	if (CDialogBar::OnCreate(p_cs) == -1) return -1;
	if (!leftDlg.Create(this)) return -1;
	return 0;
}
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void CLeftBar::OnSize(UINT type, int cx, int cy)
{
